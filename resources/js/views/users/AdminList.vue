    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="Actions" width="350">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.roles.includes('guest')" v-permission="['manage user']" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row.id, scope.row.name);">
            Delete
          </el-button>
          <el-button v-if="scope.row.roles.includes('manager')" v-permission="['manage user']" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row.id, scope.row.name);">
            Delete
          </el-button>
          <el-button v-if="scope.row.roles.includes('commercial')" v-permission="['manage user']" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row.id, scope.row.name);">
            Delete
          </el-button>
          <el-button v-if="scope.row.roles.includes('host')" v-permission="['manage user']" type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row.id, scope.row.name);">
            Delete
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      roles: ['admin', 'manager', 'commercial', 'host', 'guest'],
      nonAdminRoles: ['admin', 'manager','commercial', 'host', 'guest'],
      newUser: {},
  ...
  methods: {
    ...
    handleDelete(id, name) {
      this.$confirm('This will permanently delete user ' + name + '. Continue?', 'Warning', {
        ...
        adminResource.destroy(id).then(response => {
          this.$message({
            type: 'success',
    ...
    async handleEditPermissions(id) {
      ...
      const { data } = await adminResource.permissions(id);
      this.currentUser = {
        id: found.id,
        name: found.name,
        permissions: data,
    ...
    confirmPermission() {
      ...
      adminResource.updatePermission(this.currentUserId, { permissions: checkedPermissions }).then(response => {
        this.$message({
